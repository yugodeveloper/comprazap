...
function LandingContent() {
  ...
  const handleCancelarCompra = async () => {
    ...
    setExistingOrder(null); setOrderStatus('pending'); setItemsList([]); setStep('itens');
    setLoading(false);
  };
    ...
    <div style={{ backgroundColor: '#fafaf9', minHeight: '100vh' }}>
      <div style={containerStyle}>
        ...
        <div style={{ padding: '15px 20px' }}>
          ...
          {/* 3. CARROSSEL (FOTOS VERTICAIS) */}
          {campaign?.image_gallery?.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', overflowX: 'auto', scrollSnapType: 'x mandatory', scrollbarWidth: 'none', gap: 10 }}>
                {campaign.image_gallery.map((img: string, i: number) => (
                  <div key={i} style={{ minWidth: '85%', height: '350px', scrollSnapAlign: 'center', borderRadius: '20px', overflow: 'hidden', backgroundColor: '#f1f5f9', position: 'relative' }}>
                    <img src={img} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    <div style={{ position: 'absolute', bottom: 10, right: 10, backgroundColor: 'rgba(0,0,0,0.5)', color: 'white', padding: '4px 10px', borderRadius: '15px', fontSize: '10px', fontWeight: 900 }}>
                       {i+1} / {campaign.image_gallery.length}
                    </div>
                  </div>
                ))}
              </div>
              <p style={{ textAlign: 'center', fontSize: '9px', color: '#94a3b8', fontWeight: 900, marginTop: 10, textTransform: 'uppercase' }}>➔ Arraste para o lado para ver mais fotos</p>
            </div>
          )}

          {/* 4. DESCRIÇÃO (PRESERVANDO QUEBRAS DE LINHA) */}
                ...
                <div style={{ background: '#f0fdf4', padding: 15, borderRadius: 20, border: '1px solid #dcfce7' }}>
                  {itemsList.map((item) => ( 
                    <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 13 }}> 
                      ...
                      <button onClick={() => setItemsList(itemsList.filter(i => i.id !== item.id))} style={{ background: 'none', border: 'none', color: '#ef4444' }}>✕</button>
                    </div> 
                  ))}
                  <div style={{ textAlign: 'right', fontWeight: 900, fontSize: 16 }}>Total: R$ {itemsList.reduce((acc, curr) => acc + curr.total, 0).toFixed(2)}</div>
                  <textarea placeholder="Obs (ex: Sem canela)..." style={{width: '100%', padding: '10px', borderRadius: '10px', border: '1px solid #bbf7d0', fontSize: '12px', marginTop: 10, fontFamily: 'sans-serif'}} rows={2} value={observations} onChange={e => setObservations(e.target.value)} />
