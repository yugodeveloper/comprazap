          ...
          {step === 'itens' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div style={{ background: 'white', padding: 20, borderRadius: 25, border: '1px solid #eee' }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
                  {product?.variations?.map((v: any, index: number) => (
                    <button key={index} onClick={() => setTempSelection(v)}
                      style={{ padding: '12px 18px', borderRadius: '15px', border: '1px solid #ddd', fontSize: '13px', fontWeight: 'bold',
                ...
                }} style={{ ...btnStyle, backgroundColor: '#000', marginTop: 25 }}>ADICIONAR À LISTA</button>
              </div>
                  ...
                  {itemsList.map((item) => (
                    <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, paddingBottom: 12, borderBottom: '1px solid #e2e8f0' }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap: 15 }}>
                        ...
                        <button onClick={() => setItemsList(itemsList.filter(i => i.id !== item.id))} style={{ background: 'none', border: 'none', color: '#ef4444', fontWeight: 'bold', cursor: 'pointer' }}>✕</button>
                      </div>
                    </div>
                  ))}
          ...
          {step === 'concluido' && (
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ background: 'white', padding: 25, borderRadius: 30, border: '2px solid #f1f5f9', marginBottom: 20 }}>
                ...
                <div style={{ background: '#f8fafc', padding: 10, borderRadius: 10, fontSize: 11, wordBreak: 'break-all' }}>{campaign?.pix_key}</div>
              </div>
