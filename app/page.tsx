  ...
  if (!isLoggedIn) { 
     return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-slate-900">
        <h1 className="text-5xl font-black italic tracking-tighter mb-2 italic">CompraZap⚡</h1>
            ...
            return (
              <div key={camp.id} className={`bg-white p-5 rounded-[32px] shadow-sm border border-slate-100 space-y-4 transition-all ${isExpired ? 'bg-slate-50/80' : ''}`}>
                <div className="flex gap-3 items-center">
                  {/* MINIATURA FORÇADA - 48px FIXO COM STYLE INLINE */}
                  <div style={{ width: '48px', height: '48px', minWidth: '48px' }} className="bg-slate-100 rounded-xl overflow-hidden flex-shrink-0 border border-slate-200">
                    {camp.image_url ? (
                  ...
                  <button onClick={() => handleShare(camp)} className="p-3 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition-colors">
                    📢
                  </button>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  {isExpired ? (
                    <button onClick={() => handleReactivate(camp.id)} className="flex-1 text-[10px] font-black uppercase py-3 bg-blue-600 text-white rounded-xl">Reativar 🔄</button>
                  ) : (
