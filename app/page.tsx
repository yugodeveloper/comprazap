...
export default function PortalCondominio() {
  ...
  const handleReactivate = async (campId: string) => {
    ...
    if (!error) {
      ...
      alert("Campanha reativada por mais 7 dias! 🚀");
    }
  };
        ...
        <div className="w-full max-w-sm space-y-6">
          {view === 'phone' && (
            <div className="space-y-4 animate-in fade-in duration-500">
              ...
              <button onClick={handleCheckPhone} className="w-full bg-slate-900 text-white py-6 rounded-[30px] font-black text-xl shadow-2xl active:scale-95 transition-all">{loading ? 'CARREGANDO...' : 'ENTRAR'}</button>
            </div>
          )}
          {view === 'login' && (
            <form onSubmit={handleAuthAction} className="space-y-4 animate-in zoom-in duration-300">
              ...
              <button type="submit" className="w-full bg-slate-900 text-white py-6 rounded-[30px] font-black text-xl shadow-2xl uppercase">{loading ? 'AUTENTICANDO...' : 'CONFIRMAR'}</button>
          ...
          {view === 'signup' && (
            <form onSubmit={handleAuthAction} className="space-y-3 animate-in slide-in-from-bottom-4 duration-500">
              ...
              <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-[30px] font-black text-lg shadow-xl uppercase">{loading ? 'SALVANDO...' : 'SALVAR DADOS'}</button>
      ...
      <div className="max-w-md mx-auto space-y-10">
        ...
        <section className="space-y-4">
          ...
          {myCampaigns.map(camp => {
            ...
            return (
              <div key={camp.id} className={`bg-white p-5 rounded-[32px] shadow-sm border border-slate-100 space-y-4 ${isExpired ? 'bg-slate-50/80' : ''}`}>
                <div className="flex gap-4 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`font-black text-slate-800 truncate ${isExpired ? 'text-slate-500' : ''}`}>{camp.title}</p>
                    <div className="flex items-center gap-2">
                       <span className={`text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-tighter ${isExpired ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                        {isExpired ? 'Encerrada' : 'Ativa'}
                      </span>
                      <p className="text-[9px] font-bold text-slate-400 uppercase">
                        {isExpired ? `Fim: ${new Date(camp.expires_at).toLocaleDateString('pt-BR')}` : `Expira: ${new Date(camp.expires_at).toLocaleDateString('pt-BR')}`}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 py-3 border-y border-slate-50">
                  <div className="text-center border-r border-slate-50">
                    <p className="text-sm font-black text-slate-900">{camp.views || 0}</p>
                    <p className="text-[8px] font-black text-slate-400 uppercase">Visitas</p>
                  </div>
                  <div className="text-center border-r border-slate-50">
                    <p className="text-sm font-black text-blue-600">{totalSales}</p>
                    <p className="text-[8px] font-black text-slate-400 uppercase">Pedidos</p>
                  </div>
                  <div className="text-center">
                    <p className="text-sm font-black text-green-600">{paidSales}</p>
                    <p className="text-[8px] font-black text-slate-400 uppercase">Pagos ({hasReceipt} 📄)</p>
                  </div>
                </div>

                <div className="flex gap-2">
                  <button onClick={() => router.push(`/campanha/gestao/${camp.id}`)} className="flex-1 text-[10px] font-black uppercase py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition-all">Gerenciar</button>
                  {isExpired ? (
                    <button onClick={() => handleReactivate(camp.id)} className="flex-1 text-[10px] font-black uppercase py-3 bg-blue-600 text-white rounded-xl shadow-lg shadow-blue-100 active:scale-95 transition-all">Reativar 🔄</button>
                  ) : (
                    <button onClick={() => handleEndCampaign(camp.id)} className="px-4 text-[10px] font-black uppercase py-3 bg-red-50 text-red-500 rounded-xl hover:bg-red-100 transition-all">Encerrar</button>
                  )}
                </div>
              </div>
            );
