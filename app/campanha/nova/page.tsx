    ...
    if (editId) {
      const loadEditData = async () => {
        ...
        if (camp) {
          ...
          setMaxSales(camp.max_sales.toString());
          setHeaderImg(camp.image_url || '');
          setGallery(camp.image_gallery || []);
          if (camp.products?.[0]?.variations) {
            setVariations(camp.products[0].variations.map((v: any) => ({ name: v.name, price: v.price.toString() })));
          }
        }
      }
      loadEditData();
      ...
      <div style={{ maxWidth: '450px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase' }}>Capa da Campanha (Horizontal)</label>
            {headerImg ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase' }}>Fotos do Produto (Carrossel)</label>
          ...
          <input placeholder="Chave Pix" required style={inputStyle} value={pixKey} onChange={e => setPixKey(e.target.value)} />
